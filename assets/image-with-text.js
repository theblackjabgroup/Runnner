...
function initializeSection(section, sectionId) {
  ...
  if (prefersReducedMotion || (!enableTextAnimation && !enableButtonAnimation)) {
    ...
    if (enableTextAnimation) {
      section
        .querySelectorAll('.image-with-text-letter-animation, .date-animation, .text-animation')
        .forEach((element) => {
  ...
  if (prefersReducedMotion) {
    ...
    section
      .querySelectorAll('.image-with-text-letter-animation, .date-animation, .text-animation, .button-animation')
      .forEach((element) => {
        element.style.opacity = '1';
      });
  } else {
        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            if (enableTextAnimation) {
              ...
              const headingElements = entry.target.querySelectorAll('.image-with-text-letter-animation');
              ...
              const textContentElements = entry.target.querySelectorAll(
                '.image-with-text-text-content p.image-with-text-letter-animation'
              );
              ...
              const textElements = entry.target.querySelectorAll(
                '.image-with-text-letter-animation, .date-animation, .text-animation'
              );
