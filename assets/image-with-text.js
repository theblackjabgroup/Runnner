...
window.addEventListener('load', function() {
    // Overlay animation functionality
    const overlay = document.querySelector('.image-with-text-media-overlay');
    if (overlay) {
...
function initializeSection(section, sectionId) {
    ...
    if (prefersReducedMotion || (!enableTextAnimation && !enableButtonAnimation)) {
        ...
        if (enableTextAnimation) {
            section.querySelectorAll('.image-with-text-image-with-text-letter-animation, .date-animation, .text-animation').forEach(element => {
    ...
    if (prefersReducedMotion) {
        ...
        section.querySelectorAll('.image-with-text-image-with-text-letter-animation, .date-animation, .text-animation, .button-animation').forEach(element => {
            element.style.opacity = '1';
        });
    } else {
